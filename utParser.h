...
TEST_F(ParserTest, ListAsStruct) {
  ...
  ASSERT_EQ("1",t->args(0)->symbol());
  ASSERT_EQ("[]",t->args(1)->symbol());
...
TEST_F(ParserTest, ListAsStruct2) {
  ...
  Term *t = parser.createTerm();
   ...
   ASSERT_EQ("2",t->args(0)->symbol());
   ASSERT_EQ(".(1, [])",t->args(1)->symbol());
...
TEST_F(ParserTest, parseStructOfStructAllTheWay2) {
  ...
  Parser parser(scanner);
  vector<Term*> t = parser.getArgs();
  ASSERT_EQ("s(s(s(s(1))))", t[0]->symbol());
  ASSERT_EQ("b(1, 2, 3)", t[1]->symbol());
}
